"""Трассировка DOT/TRX, вариант B, 5 лет (без отмен, с колонками продано/куплено)."""
import warnings
...
def trace():
    ...
    print("=" * 170)
    print("ТРАССИРОВКА {} / {} (вариант B, 5 лет)".format(TICKER_X, TICKER_Y))
    print("=" * 170)
    ...
    for i in range(1, len(z)):
        ...
        if v >= level_X * (1 + STEP):
            if shares_x > 0:
                if CHECK_PROFIT and p_x < x_ref * 1.01:
                    cancelled += 1
                else:
                    sold_x = shares_x
                    sell = shares_x * p_x
        ...
        # Y → X
            ...
            if shares_y > 0:
                if CHECK_PROFIT and p_y < y_ref * 1.01:
                    cancelled += 1
                else:
                    sold_y = shares_y
                    sell = shares_y * p_y
    ...
    print("Комиссия: {:.2f}$".format(commission_total))

    final_px = float(px.iloc[-1])
    final_py = float(py.iloc[-1])
    if shares_x > 0:
        final_value = shares_x * final_px
        print("Финал: X={:.6f} монет * {:.6f} = {:.2f}$".format(shares_x, final_px, final_value))
    elif shares_y > 0:
        final_value = shares_y * final_py
        print("Финал: Y={:.6f} монет * {:.6f} = {:.2f}$".format(shares_y, final_py, final_value))
    else:
        final_value = cash
        print("Финал: cash={:.2f}$".format(cash))
